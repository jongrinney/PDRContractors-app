cms.initForms = function() {
	...
	$formContainers.each(function() {
		...
		$.getJSON('https://pdrcontractors.com/forms/' + id + '.json', function(data) {
			$form.html(
				'<form class="styledForm mostRecent" method="post" data-type="' +
					data.formType +
					'" data-cost="' +
					data.cost +
					data.defaultAmount +
			...
			$form
				.find('input[type="radio"][value="Other"]+label>input')
				.keyup(function() {
					$(this)
						.parent()
						.prev()
			...
			if (typeof cms.attachFormHandlers !== 'undefined') cms.attachFormHandlers();
...
cms.attachFormHandlers = function(processor) {
	...
	$('form:not(.live) :input').change(function() {
		...
		$.post('https://pdrcontractors.com/cms/modules/forms/json/getFormPriceTotal.json.php', $form.serialize(), function(data) {
			if (data.message == 'good') {
				...
				if (typeof data.data.discount === 'undefined' || (data.data.discount == 0 && data.data.promocodeMessage == '')) $form.find('.priceTotal .discount').hide();
					...
					if (data.data.discount != 0) {
						...
						$form
							.find('.priceTotal .discount td')
							.html('-' + format_money(data.data.discount) + '<br><em>' + data.data.promocode.toUpperCase() + ' - ' + data.data.promocodeMessage + '</em>');
					} else {
	...
	if (typeof processor === 'undefined' || processor == '') {
		$('form.styledForm:not(.live)')
			.addClass('live')
			.attr('target', 'formSubmitter')
			.unbind('submit')
			.submit(function() {
				swal.showLoading();
				...
				if (
					...
					$(this)
						...
						.css('display', 'block');
					$(this)
						.find(':input[data-required]:hidden')
						.prop('required', false);
					return;
				}
				$(this)
							...
							try {
								...
								if (data.messageTitle != 'N/A') {
									swal({
										...
										else if (typeof data.doRefresh !== 'undefined' && data.doRefresh) window.location.reload();
									});
								} else {
									if (typeof data.link !== 'undefined' && data.link != '') window.location.href = data.link;
									else if (typeof data.doRefresh !== 'undefined' && data.doRefresh) window.location.reload();
								...
								$(formElem)
									.find(':input:not([type="radio"]):not([type="checkbox"]):not([type="button"]):not([type="submit"]):not([type="hidden"])')
									.val(''); //formElem.reset();
								$(formElem)
									.find(':input[type="radio"], :input[type="checkbox"]')
									.prop('checked', false); //formElem.reset();
							} catch (e) {
	...
	$('a[href^="#cloneFields["]')
		...
		.click(function(e) {
			...
			var $form = $(this).closest('form');
			for (var i in fields)
				$form.find('input[name="' + fields[i] + '[]"]').val(
					$form
