window.doPasswordReset = function(){
	Swal({
		...
		if (result.value) {
			setTimeout(function(){Swal.showLoading();},200);
			$.post('/cms/modules/users/json/doPasswordReset.json.php',$('#loginForm').serialize(),function(){
				Swal.hideLoading();
				Swal(
					'',
					'If an account was found with that username, you will receive an email with a temporary password. Enter it in the password box to continue.',
					'success'
				)
			});
		}
		...
		if(data.message=='good'){
			...
			if($('#loginForm').children().length==3){
				Swal({
					...
					text: "Please enter a new permenant password below.",
					type: 'success'
				});
				$('#loginForm>label').last().after('<label>New Password<input name="new_password" type="password" autocomplete="current-password"></label>');
			}
		}
			...
			if($('#loginForm').children().length==3){
				$('#loginForm>label').last().after('<input class="button" type="button" onclick="doPasswordReset()" value="Forgot Password" style="width:100%">');
			}
		}
		else if($('#loginForm [name="username"]').val() != '')
			alert(data.message);
	});
	return false;
});
